...
for dam, dam_id in zip(name, id_list):
    ...
    feature = [f'{dam_id}_inflow mcm', f'{dam_id}_volume mcm', f'{dam_id}_outflow mcm']
    x = df[feature]

    x[[f'{dam_id}_inflow mcm', f'{dam_id}_outflow mcm']] = x[[f'{dam_id}_inflow mcm', f'{dam_id}_outflow mcm']] * (10**6) / delta_t

    y = df[f'{dam_id}_outflow mcm'] * (10**6) / delta_t # convert from mcm/day to m^3/s
    dv = (df[f'{dam_id}_inflow mcm'] - df[f'{dam_id}_outflow mcm'])
    ...
    delta_v_train_arr = delta_v_train.values
    ...
    pmp.optimize(objective_function, n_trials=2000, show_progress_bar=True)
